...
import { Placement } from '../../types/maintenance';
...
import { RackMaintenanceComponent } from '../components/common/rack-maintenance.component';
...
@Component({
  ...
  imports: [DecimalPipe, ReactiveFormsModule, MatButtonModule, MatCheckboxModule, MatFormFieldModule, MatInputModule, MatSelectModule,
    ConstraintBadgeComponent, RackMaintenanceComponent, LucideAngularModule],
  ...
  template: `
    <div class="page planner-page">
      ...
      <app-rack-maintenance [racks]="racks()" [placements]="placements()" (changed)="load()" />
      ...
      @if (selected(); as active) {
        @if (active.maintenance) {
          <section class="maint-readback panel">
            <div class="panel-header"><h2>Migration record &middot; {{ active.maintenance.maintenance_rack_code }}</h2><span class="secondary">{{ active.maintenance.zone_code }} &middot; {{ active.algorithm_version }} &middot; frozen draft v{{ active.version }}</span></div>
            <div class="maint-readback-grid">
              <div><h3>Before maintenance</h3>@for (view of active.maintenance.before; track view.load_id + '-' + view.rack_id) {<div class="rb-line" [class.source]="view.rack_code === active.maintenance.maintenance_rack_code"><span>{{ view.load_name }}</span><strong>{{ view.rack_code }}</strong></div>}</div>
              <div><h3>After migration</h3>@for (view of active.maintenance.after; track view.load_id + '-' + view.rack_id) {<div class="rb-line" [class.dest]="movedRackCodes().has(view.rack_code)"><span>{{ view.load_name }}</span><strong>{{ view.rack_code }}</strong></div>}</div>
            </div>
            <div class="rb-moves">@for (move of active.maintenance.moves; track move.load_id) {<span class="rb-chip"><lucide-icon name="truck" [size]="12" />{{ move.load_name }}: {{ move.from_rack_code }} &rarr; {{ move.to_rack_code }}</span>}</div>
          </section>
        }
...
export class PlannerPage {
  ...
  readonly placements = signal<Placement[]>([]);
  ...
  readonly movedRackCodes = computed(() => new Set((this.selected()?.maintenance?.moves ?? []).map((move) => move.to_rack_code)));
  ...
  load(): void { forkJoin({scenarios: this.scenarioApi.list(), racks: this.rackApi.list(), loads: this.loadApi.list(), placements: this.rackApi.placements()}).subscribe(({scenarios, racks, loads, placements}) => { this.scenarios.set(scenarios.items); this.racks.set(racks.items); this.loads.set(loads.items); this.placements.set(placements.items); const current = this.selected(); const selected = scenarios.items.find((item) => item.id === current?.id) ?? scenarios.items[0] ?? null; this.store.select(selected); if (this.selectedLoadIds().size === 0) this.selectedLoadIds.set(new Set(loads.items.filter((item) => item.load_status === 'ready').map((item) => item.id))); }); }
